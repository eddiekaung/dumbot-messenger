...
function handleCommand(command, fromUserId, groupInfo, messageLiteral, api = gapi) {
    ...
    if (co["help"].m) { // Check help first to avoid command conflicts
        ...
        if (co["show"].m[1].toLowerCase() === 'akh') { // Parameter specified
            sendFileFromDir("media/AKH", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'ahp') {
            sendFileFromDir("media/AHP", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'ah') {
            sendFileFromDir("media/AH", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'athk') {
            sendFileFromDir("media/ATHK", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'ec') {
            sendFileFromDir("media/EC", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'emh') {
            sendFileFromDir("media/EMH", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'htm') {
            sendFileFromDir("media/HTM", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'ksl') {
            sendFileFromDir("media/KSL", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'mb') {
            sendFileFromDir("media/MB", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'nwl') {
            sendFileFromDir("media/NWL", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'pmo') {
            sendFileFromDir("media/PMO", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'ppk') {
            sendFileFromDir("media/PPK", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'skb') {
            sendFileFromDir("media/SKB", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'tha') {
            sendFileFromDir("media/THA", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'thha') {
            sendFileFromDir("media/THHA", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'wpa') {
            sendFileFromDir("media/WPA", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'yya') {
            sendFileFromDir("media/YYA", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'zln') {
            sendFileFromDir("media/ZLN", threadId);
        } else if (co["show"].m[1].toLowerCase() === 'znl') {
            sendFileFromDir("media/ZNL", threadId);
        } else { // No parameter passed; send random xkcd
            // Get info of most current xkcd to find out the number of existing xkcd (i.e. the rand ceiling)
            sendMessage(`No photo saved for ${co["show"].m[1]}`, threadId);
        };
    } else if (co["kpop"].m) {
        const search = co["kpop"].m[1];
        request.get(`https://www.reddit.com/r/kpics/search.json?q=${encodeURIComponent(search)}&restrict_sr=on&sort=relevance&t=all`, (err, res, body) => {
            if (!err) {
                ...
                if (results) {
                  ...
                  if(chosen) {
                    ...
                    const link = data.url;
                    while (link == 'self.kpics' || data.over_18 == true) {
                      const chosen = posts[[Math.floor(Math.random() * posts.length)]]
                      const data = chosen.data;
                      const link = data.url;
                    };
                    if (link.indexOf("imgur.com/a/") > -1) {
                      const imgurId = link.substring(link.indexOf("a/")+2);
                      sendFileFromImgur(imgurId, `${data.title}`, threadId);
                    } else if (link.indexOf("gfycat") > -1) {
                      var gfycat = link.substring(link.indexOf("com")+4);
                      sendFileFromUrl(`https://thumbs.gfycat.com/${gfycat}-mobile.mp4`, 'media/kpop.mp4', `${data.title}`, threadId);
                    } else if (link.indexOf("v.redd") > -1) {
                      sendFileFromUrl(data.media.reddit_video.fallback_url, 'media/kpop.mp4', `${data.title}`, threadId);;
                    } else {
                      sendFileFromUrl(link, `media/kpop.jpg`, `${data.title}`, threadId);
                    }
                  } else {
                      sendError(`No results found for ${search}`, threadId);
                    }
  ...
  }  else if (co["depress"].m) {
        request.get(`https://www.reddit.com/r/2meirl4meirl/top.json?t=week`, (err, res, body) => {
            if (!err) {
                const results = JSON.parse(body);
                if (results) {
                  const posts = results.data.children;
                  ...
                  const data = chosen.data;
                  const link = data.url;
                  while (link == 'self.22meirl4meirl' || data.over_18 == true) {
                    const chosen = posts[[Math.floor(Math.random() * posts.length)]]
                    const data = chosen.data;
                    const link = data.url;
                  };
                    sendFileFromUrl(link, `media/depress.jpg`, ``, threadId);
                } else {
                    sendError(`No results found`, threadId);
                }
            } else {
    ...
    } else if (co["polandball"].m) {
        request.get(`https://www.reddit.com/r/polandball.json`, (err, res, body) => {
            if (!err) {
                const results = JSON.parse(body);
                if (results) {
                  const posts = results.data.children;
                  ...
                  const data = chosen.data;
                  const link = data.url;
                  while (link == 'self.polandball' || data.over_18 == true) {
                    const chosen = posts[[Math.floor(Math.random() * posts.length)]]
                    const data = chosen.data;
                    const link = data.url;
                  };
                    sendFileFromUrl(link, `media/poland.jpg`, `${data.title}`, threadId);
                } else {
                    sendError(`No results found`, threadId);
                }
            } else {
        ...
        request.get(`https://www.reddit.com/r/lossedits/top.json?t=week`, (err, res, body) => {
            if (!err) {
                const results = JSON.parse(body);
                if (results) {
                  const posts = results.data.children;
                  ...
                  const data = chosen.data;
                  const link = data.url;
                  while (link == 'self.loss' || data.over_18 == true) {
                    const chosen = posts[[Math.floor(Math.random() * posts.length)]]
                    const data = chosen.data;
                    const link = data.url;
                  };
                  if (link.indexOf("imgur.com/a/") > -1) {
                    ...
                    sendMessage(link, threadId);
                  } else {
                    sendFileFromUrl(link, `media/loss.jpg`, `${data.title}`, threadId);
                  }
                } else {
                    sendError(`No results found`, threadId);
                }
        ...
        request(`http://api.wolframalpha.com/v1/result?appid="P2HK5G-5X5RV298Q4"&i=${encodeURIComponent(query)}`, (err, res, body) => {
        ...
        sendContentsOfFile("changelog.txt", threadId);
        ...
        request(`http://api.openweathermap.org/data/2.5/weather?appid=912befada2ed9dfd287d9dbbf4a2c8c6&q=${city}&units=imperial`, (err, res, body) => {
            if (!err && res.statusCode == 200) {
        ...
        sendMessage(`${groupInfo.members}asdasda${groupInfo.names}`, threadId);
    } else if (co["thanos"].m) {
...
function handlePings(msg, senderId, info) {
    ...
    const parsedMessage = pingData.parsed;
    if (pingUsers) {
        for (let i = 0; i < pingUsers.length; i++) {
            ...
            if (parsedMessage.length > 0) { // Message left after pings removed – pass to receiver
                        ...
                        if (!err) {
                            ...
                            for (let id in userData) {
                                if (userData.hasOwnProperty(id) && id != config.bot.id) { // Very important to not add bot to participants list
                                    info.members[userData[id].name.toLowerCase()] = id;
                                    info.names[id] = userData[id].name;
                                    info.birthday[id] = userData[id].isBirthday;
                                }
                            }
...
// Returns a string of the current time in Myanmar Time Zone
function getTimeString() {
    const offset = +6.5; // Myanmar
    const d = new Date();
...
function sendFileFromDir(dir, threadId) {
  fs.readdir(dir, (err, filenames) => {
    if (!err) {
      const chosen = filenames[Math.floor(Math.random() * filenames.length)];
      sendFile(`${dir}/${chosen}`, threadId);
    } else {
      console.log(err);
    }
  });
}
exports.sendFileFromDir = sendFileFromDir;


function sendFileFromImgur(imgurId, title, threadId) {
  request.get({
    url: `https://api.imgur.com/3/album/${imgurId}/images`,
    headers: {
      'Authorization': 'Client-ID fa64eb880538f14'
    }
  }, (err, res, body) => {
    if (!err) {
      var data = JSON.parse(body).data;
      var chosen = data[Math.floor(Math.random() * data.length)];
      sendFileFromUrl(chosen.link, 'media/kpop.jpg', title, threadId);
    }
  });
